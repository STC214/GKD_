...
当前进度：阶段 0.5～6 已完成，阶段 7 进行中。APK 已固定 libsu 6.0.0 并完成普通非 daemon RootService 的最小握手真机验收：远端 UID 0、协议 1，Binder 死亡保留具体原因并可用新 PID 重连，强停/更新/卸载无 daemon 残留。第二切片已加入 Parcelable 数值化 `tap/swipe` 请求、逐调用身份校验、displayId/有限坐标/窗口边界/时长校验和明确结果码，但尚未接入规则动作链。App 115/115、Selector 18/18、Release/R8/Vital Lint 已通过。

最新停点已经记录在 `docs/current-progress.md` 的“本轮开发停点”；恢复开发时从阶段 7 的 Root → Shizuku → A11y 显式桥接开始，不重复 RootService 握手和结构化输入参数模型。
