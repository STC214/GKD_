...
当前进度：阶段 0.5～4 已完成。阶段 4“前台与焦点窗口融合”已在 Android 16 真机完成普通应用、SystemUI、输入法、权限弹窗和真实画中画验收；Task/窗口冲突采用一次 150ms 有界确认，根节点持续错配时每个上下文最多补查一次，只有确认后的普通应用窗口可以进入规则查询和动作提交。输入法与 PiP 即使被系统标为非 focused/active 也会被显式识别，权限控制器同时检查 Task 和 Window 来源。

最新停点已经记录在 `docs/current-progress.md` 的“本轮开发停点”；恢复开发时直接进入阶段 5 的窗口 generation、短时恢复与节点新鲜度改造。
