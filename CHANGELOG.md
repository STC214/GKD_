...
- KernelSU/SukiSU 模块改为纯保活：移除内置 APK、自动安装/更新、权限授予、自动开启无障碍和卸载 App；模块只处理已安装 GKD 的后台 AppOps、电池白名单、后台启动及低频恢复。
- 打包脚本不再运行 Gradle 或接受 APK 路径，并用固定 7 文件白名单校验 ZIP；移除模块恢复后台 AppOps、清理电池白名单并保留 GKD、权限与全部数据。
- 保活目标新增 `GKD_USER_ID=0`，包检测、进程 UID、服务启动、AppOps 和设置读取使用同一用户；日志大小配置会对非法值回退并应用最小值。
- 启动逻辑改为三态循环：未安装 GKD 时模块低频休眠；已安装但未运行时以最高 30 秒退避持续拉起，直到确认目标 UID 进程；成功后才进入低频保活，卸载 App 后重新回到休眠态。
- 系统服务等待不再超时退出；启动成功需连续 3 秒确认目标 UID 进程稳定存在。保活、策略刷新和日志上限均增加安全最大值，超长或非法配置不会造成整数溢出或近似永久休眠。
...
- 模块管理器动作按钮会检查 GKD 是否由用户单独安装；缺失时只提示，不执行安装。

# 更新内容

- 阶段 7 已整体提交为 `f36a0d71`；正式 Release `1.12.1-f36a0d7` 从干净工作区完成 R8/Vital Lint 构建并通过 v2 签名验证。
