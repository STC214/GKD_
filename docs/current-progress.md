...
- 当前代码提交：`f36a0d71`；阶段 7 的 AIDL、Root 生命周期、动作桥、Task 采样、测试及文档已作为同一提交边界落库。
- Release APK：`app/build/outputs/apk/gkd/release/app-gkd-release.apk`
- 正式版本：`1.12.1-f36a0d7`（versionCode 92）
- 当前本地 APK SHA-256：`8ABB34862065D51E7D76027B6F3E90A150D8BF0EA546748D1637A63EFC28B436`
- APK 大小：3,373,080 字节。
...
- KernelSU/SukiSU 模块已收缩为纯保活：ZIP 不再内置 APK，脚本不再安装、更新、授予权限、开启无障碍或卸载 GKD，只保留已安装 App 的后台 AppOps、电池白名单、后台启动和低频恢复。启动逻辑固定为三态循环：目标用户未安装 GKD 时模块低频休眠；已安装但未运行时以最高 30 秒退避持续拉起，连续 3 秒按包 UID 确认进程稳定后才进入保活；App 被卸载后重新回到休眠态。审查风险已收口：系统服务持续等待，数值配置具有安全上下限且超长输入不会进入整数比较。新版模块 `202607182357` 已通过 shell 语法、LF/无 BOM、ZIP 固定 7 文件、零 APK、源码/归档一致性及风险修复内容检查；产物 SHA-256 为 `5262C4697171D9E420F5A902F85B6305E13EF17AA8BA640C74B76F2241F9C22E`，按用户要求不做真机验证。
- 最后完成项：阶段 7 全部代码与 Android 16 真机收口并提交为 `f36a0d71`。拒绝/无回调不循环、撤权后的进程死亡收口、A11y 回退、覆盖更新和重新授权恢复均已通过；SukiSU 不追溯结束既有 root 进程的边界已在界面和文档明确。最终 App 123/123、Selector 18/18 通过；从干净提交重新执行 Release/R8/Vital Lint 成功，APK v2 签名有效，SHA-256 为 `8ABB34862065D51E7D76027B6F3E90A150D8BF0EA546748D1637A63EFC28B436`。
...
- 当前 Release APK 为干净提交构建 `1.12.1-f36a0d7`；阶段 7 代码与此前文档均已提交。独立 Debug 包已完成真机验收并卸载；该正式 APK 尚未在本轮重新连接手机安装，留待后续冒烟测试。
