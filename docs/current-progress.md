...
- KernelSU/SukiSU 模块已收缩为纯保活：ZIP 不再内置 APK，脚本不再安装、更新、授予权限、开启无障碍或卸载 GKD，只保留已安装 App 的后台 AppOps、电池白名单、后台启动和低频恢复。审查后统一 `GKD_USER_ID` 上下文、按包 UID 检测进程、校验日志上限、卸载时恢复后台 AppOps，并将归档收紧为固定 7 文件白名单。新版模块 `202607182116` 已通过 shell 语法、LF/无 BOM、ZIP 路径及零 APK 条目检查；产物 SHA-256 为 `79B0EE936BDEFC9B49AEBC1E91C5830CCBD80CDCB2C5E2E6A7504A733A4AE77E`，按用户要求不做真机验证。
